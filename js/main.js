...
const boardSquares = document.getElementsByClassName("chess-board__square");
...
function fillBoardSquaresArray() {
  ...
  for (let i = 0; i < boardSquares.length; i++) {
    ...
    if (square.querySelector(".chess-piece")) {
      color = square.querySelector(".chess-piece").getAttribute("color");
      pieceType = square.querySelector(".chess-piece").classList[1];
      pieceId=square.querySelector(".chess-piece").id;
    } else {
      color = "blank";
      pieceType = "blank";
      pieceId ="blank";
...
function drop(ev) {
  ...
  if (
    ...
    for (let i = 0; i < children.length; i++) {
        if (!children[i].classList.contains('chess-coordinate')) {
...
function getPossibleMoves(startingSquareId, piece, boardSquaresArray) {
  ...
  if  (pieceType=="chess-piece_knight") {
  ...
  if  (pieceType=="chess-piece_bishop") {
  ...
  if  (pieceType=="chess-piece_queen"){
...
function isKingInCheck(SquareId, pieceColor, boardSquaresArray) {
  ...
  for (let squareId of legalSquares) {
    ...
    if (
      (pieceProperties.pieceType == "chess-piece_rook" ||
        pieceProperties.pieceType == "chess-piece_queen") &&
  ...
  for (let squareId of legalSquares) {
    let pieceProperties = getPieceAtSquare(squareId, boardSquaresArray);
    if (
      (pieceProperties.pieceType == "chess-piece_bishop" ||
        pieceProperties.pieceType == "chess-piece_queen") &&
      pieceColor != pieceProperties.color
    ) {
      return true;
  ...
  for (let squareId of legalSquares) {
    let pieceProperties = getPieceAtSquare(squareId, boardSquaresArray);
    if (
      pieceProperties.pieceType == "chess-piece_knight" &&
      pieceColor != pieceProperties.color
    ) {
      return true;
    }
  ...
  for (let squareId of legalSquares) {
    let pieceProperties = getPieceAtSquare(squareId, boardSquaresArray);
    if (
      pieceProperties.pieceType == "chess-piece_pawn" &&
      pieceColor != pieceProperties.color
    ) {
      return true;
    }
  ...
  for (let squareId of legalSquares) {
    let pieceProperties = getPieceAtSquare(squareId, boardSquaresArray);
    if (
      pieceProperties.pieceType == "chess-piece_king" &&
      pieceColor != pieceProperties.color
    ) {
      return true;
    }
...
function isMoveValidAgainstCheck(legalSquares,startingSquareId,pieceColor,pieceType){
  ...
  legalSquaresCopy.forEach((element) => {
    ...
    if (pieceType != "chess-piece_king" && isKingInCheck(kingSquare, pieceColor, boardSquaresArrayCopy)) {
      legalSquares = legalSquares.filter((item) => item !== destinationId);
    }

    if (pieceType == "chess-piece_king" && isKingInCheck(destinationId, pieceColor, boardSquaresArrayCopy)) {
      legalSquares = legalSquares.filter((item) => item !== destinationId);
    }
